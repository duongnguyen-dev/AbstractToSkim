...
def get_data_ready(train_ds, val_ds, test_ds):
    try:
        ...
        train_line_numbers = tf.one_hot(train_df['line_number'].to_numpy(), depth=CFG.LINE_NUMBER_DEPTH)
        val_line_numbers = tf.one_hot(val_df['line_number'].to_numpy(), depth=CFG.LINE_NUMBER_DEPTH)
        test_line_numbers = tf.one_hot(test_df['line_number'].to_numpy(), depth=CFG.LINE_NUMBER_DEPTH)

        # Input 3: Total line 
        train_total_lines = tf.one_hot(train_df['total_lines'].to_numpy(), depth=CFG.TOTAL_LINE_DEPTH)
        val_total_lines = tf.one_hot(val_df['total_lines'].to_numpy(), depth=CFG.TOTAL_LINE_DEPTH)
        test_total_lines = tf.one_hot(test_df['total_lines'].to_numpy(), depth=CFG.TOTAL_LINE_DEPTH)
        ...
        train_dataset = tf.data.Dataset.zip(train_tribrid_data, train_tribrid_label).batch(CFG.BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
        ...
        val_dataset = tf.data.Dataset.zip(val_tribrid_data, val_tribrid_label).batch(CFG.BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
        ...
        test_dataset = tf.data.Dataset.zip(test_tribrid_data, test_tribrid_label).batch(CFG.BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
